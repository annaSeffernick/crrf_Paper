...
seed1 <- 5254
N <- 100 # number of observations (50, 100, 200)
umax=0.55 # control censoring rate (increase censorign by decreasing umax) (umax=1 ~ 50% censoring; umax=0.55 ~65% censoring; umax=0.25 ~ 80% censoring)
p=0.5 # control event rate for event of interest (0.5, 0.3)
...
for(k in 1:B){
  ...
  firth.fit <- crrf(form,failcode,dset,T,T, eps=1e-10)
...
bias.list <- list(crr.bias, firth.bias, beta.dif)
names(bias.list) <- c("CRR", "Firth", "Difference")
